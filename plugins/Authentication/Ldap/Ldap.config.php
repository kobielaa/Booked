...
$conf['settings']['host'] = 'rinf.local'; // comma separated list of ldap servers such as mydomain1,localhost
...
$conf['settings']['binddn'] = 'CN=booked,OU=Service Accounts,DC=rinf,DC=local';	// The distinguished name to bind as (username). If you don't supply this, an anonymous bind will be established.
$conf['settings']['bindpw'] = 'UA?X^<R.]Xzt6RfL';	// Password for the binddn. If the credentials are wrong, the bind will fail server-side and an anonymous bind will be established instead. An empty bindpw string requests an unauthenticated bind.
$conf['settings']['basedn'] = 'dc=rinf,dc=local';	// LDAP base name (eg. dc=example,dc=com)
$conf['settings']['filter'] = '(&(objectCategory=Person)(sAMAccountName=#{username})(mail=*)(primaryGroupID=513)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))';	// Default search filter
